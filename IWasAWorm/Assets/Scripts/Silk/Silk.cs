...
public class Silk : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        farEnd.TempMass.name = "FarEnd TempMass of " + name;
        Ends[0] = farEnd;
    ...
    public void NewSpit(Vector2 respawnPos, Vector2 velocity, Worm worm, float range)
    {
        ...
        FarEnd.Item = FarEnd.TempMass.GetComponent<Rigidbody2D>();
        FarEnd.Anchor = Vector2.zero;
        NearEnd.State = SilkEndState.Held;
        NearEnd.Item = worm.GetComponent<Rigidbody2D>();
        ...
        tempMass.GetComponent<Rigidbody2D>().velocity = velocity;
        tempMass.GetComponent<Rigidbody2D>().gravityScale = 0;
    ...
    public void BreakUp()
    {
        ...
        NearEnd.Item = NearEnd.TempMass.GetComponent<Rigidbody2D>();
    ...
    {
        var lastLine = Segments[i].LastLine;//记录之前的lastLine
        ...
        newSegment.LastLine = lastLine.GetNearPart(nearPartDistance / totalDistance);//Segments[i+1].LastLine应该变成之前的近的一部分

        Segments[i].CreateJoint(farPartLength);
        Segments[i].LastLine = lastLine.GetFarPart(farPartDistance/totalDistance);//Segments[i].LastLine应该变成之前的远的一部分

        Segments[i].CheckSeperate();
        newSegment.CheckSeperate();
    }
    /// <summary>
    ...
    void Update()
    {
        ...
        if (FarEnd.State == SilkEndState.Spit && NearEnd.State == SilkEndState.Held)
        {
            ...
            if (curFarEndPosWorld != _lastFarEndPosWorld)
            {
                ...
                else if ((curFarEndPosWorld - _respawnPos).magnitude >= Range)//达到射程
                {
                    #region FarEnd.Spit->Free
                    FarEnd.State = SilkEndState.Free;
                    FarEnd.TempMass.GetComponent<Rigidbody2D>().gravityScale = 1;
                    FarEnd.TempMass.GetComponent<Rigidbody2D>().drag = 2;
                    FarEnd.TempMass.AddComponent<CircleCollider2D>().radius = 0;
                    FarEnd.TempMass.SetActive(true);
                    FarEnd.Item = FarEnd.TempMass.GetComponent<Rigidbody2D>();
                    FarEnd.Anchor = Vector2.zero;
                    Segments[0].CreateJoint(Segments[0].Distance);
                    var segmentDir = (Segments[0].FarPoint.Position - Segments[0].NearPoint.Position).normalized;
                    FarEnd.TempMass.GetComponent<Rigidbody2D>().velocity -= Vector2.Dot(FarEnd.TempMass.GetComponent<Rigidbody2D>().velocity, segmentDir) *
                                                          segmentDir;
    ...
    void FixedUpdate()
    {
        foreach (var silkSegment in Segments)
        {
            silkSegment.FixedUpdate(Time.fixedDeltaTime);
        }
    }
